...
function DeliveryBoyDashboard({earning}:{earning:number}) {
    ...
    useEffect(():any=>{
        ...
        socket.on("new-assignment", (deliveryAssignment)=>{
            console.log("delivery",deliveryAssignment )
        ...
        try {
            ...
            if(result.data.active){
                ...
                setUserLocation({
                    ...
                    longitude:result.data.assignment.order.address.lontitude
                })
            }
        } catch (error) {
            console.log(error)     
    ...
    if(!activeOrder && assignment.length === 0){
        const todayEarning = [
            {name:"Today",
            earning,
            // logic return in deliveryBoy component
            ...
            <div className="flex items-center justify-center min-h-screen bg-linear-to-br from-white to-green-50 p-6">
                <div className="max-w-md w-full text-center">
                    ...
                    <div className="bg-white border rounded-xl shadow-xl p-6">
                        <h2 className="font-medium text-green-700">Today's Performance</h2>
            ...
            <div className="p-4 pt-[120px] min-h-screen bg-gray-50">
                <div className="max-w-3xl mx-auto">
                    ...
                    <div className="mt-6 bg-white rounded-xl border shadow p-6">
                        {!activeOrder.order.deliveryOtpVefication && !showOtpBox && (
                        <button
                        onClick={sendOtp}
                        className="w-full py-4 bg-green-600 text-white text-center rounded-lg cursor-pointer"
                        >{sendOtpLoading ? <Loader className="animate-spin text-center" size={16}/> : "Mark as Delivered"}</button>
                        )}

                        {
                            ...
                            <div className="mt-4">
                                ...
                                className="w-full mt-4 bg-blue-600 text-white py-3 text-center rounded-lg">
                                    {verifyOtpLoading ? <Loader className="animate-spin text-center " size={16}/> : "Verify Otp"}
                                </button>
                        ...
                        {activeOrder.order.deliveryOtpVefication && <div
