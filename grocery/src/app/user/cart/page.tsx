...
import { decreaseQuality, increseQuality, removeFromCart } from "@/redux/cartSlice";
import { useRouter } from "next/navigation";


const CartPage = () => {

    const { cartData, finalTotal, deliveryFee, subTotal } = useSelector((state:RootState) => state.cart)
      ...
      {cartData.length == 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    {cartData.map((item)=>(
                        ...
                        >
                            ...
                            rounded-xl overflow-hidden bg-gray-50">
                                <Image className="object-contain p-3 transition-transform duration-300 hover:scale-105"
                                src={item?.image} alt={item.name} fill/>
                            </div>

                            <div className="mt-4 sm:mt-0 sm:ml-4 flex-1 text-center sm:text-left">
                                <h3 className="text-base sm:text-lg font-semibold text-gray-600 line-clamp-1">
                                    {item.name}
                                </h3>
                            ...
                            bg-gray-50 px-3 py-2 rounded-full">
                                 ...
                                 onClick={()=>dispatch(decreaseQuality(item._id))}
                                 >
                                 ...
                                 <button className="bg-white p-1.5 rounded-full hover:bg-green-100 transition-all
                                 border border-gray-200"
                                 onClick={()=>dispatch(increseQuality(item._id))}
                                 >
            ...
            >
                ...
                {/* checkout button */}
                <motion.button className="w-full mt-6 bg-green-600 text-white py-3 rounded-full
                hover:bg-green-700 transition-all font-semibold text-sm sm:text-2xl-base cursor-pointer"
                whileTap={{scale:0.95}}
