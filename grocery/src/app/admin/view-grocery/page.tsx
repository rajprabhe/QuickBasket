...
import { ArrowLeft, IndianRupee, Loader, Package, Pencil, Search, Upload, X } from "lucide-react"
...
function ViewGrocery() {
    ...
    const [fillterd, setFillterd] = useState<IGrocery[]>()
  ...
  return (
    <div className="pt-4 w-[95%] md:w-[85%] mx-auto pb-20">
        ...
        >
            <button
            onClick={()=>router.back()}
            className="flex items-center justify-center gap-2 bg-green-100 hover:bg-green-200 text-green-700
            font-semibold px-4 py-2 rounded-full transition w-full sm:w-auto"
            ><ArrowLeft size={18}/><span>Back</span></button>
            <h1 className="text-2xl md:text-3xl font-extrabold text-green-700 flex items-center
            justify-center gap-2"> <Package size={28} className="text-green-600"/>
                Manage Groceries
            </h1>
        </motion.div>
        ...
        <div className="space-y-4">
            {fillterd?.map((g,i)=>(
                ...
                border-gray-100 flex flex-col sm:flex-row items-center sm:items-start gap-5 p-5
                transition-all"
                >
                    ...
                    <div className="flex-1 flex flex-col justify-between w-full">
                        ...
                        sm:justify-between gap-2">
                            ...
                            <button className="bg-green-600 text-white px-4 py-2 rounded-lg font-semibold flex items-center justify-center gap-2 hover:bg-green-700
        ...
        <AnimatePresence>
            ...
            >
                ...
                border border-gray-200 group">
                    ...
                    />}
                    <label htmlFor="imageUpload" className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex 
                ...
                <div className="flex justify-end gap-3 mt-6">
                    ...
                    disabled={loading}
                    >
                       {loading ? <Loader size={14} className="animate-spin w-5 h-5"/> : "Edit Grocery"} 
                    </button>
                    <button className="px-4 py-2 rounded-lg bg-red-600 text-white flex items-center gap-2
                    hover:bg-red-700  transition cursor-pointer"
                    onClick={handleDelete}
                    disabled={deleteloading}
                    >
                       {deleteloading ? <Loader size={14} className="animate-spin w-5 h-5"/> : " Delete Grocery"} 
